...
function pageNavHrefAttr(index){
	if(index==1){
		...
		$('#prevPage').addClass('disablenav');
		$('#nextPage').removeClass('disablenav');
		$('#subdiv').css({
			'display': 'none'
		});
	...
	if(index==2){
		...
		$('#prevPage').removeClass('disablenav');
		$('#nextPage').removeClass('disablenav');
		$('#subdiv').css({
			'display': 'none'
		});
...
function dataListInit(data){
	...
	if(data.list){
		for(var k in data.list){
			...
			if(type == "1"){
				$('#firstPage .answers').append('<div class="checkbox"><label><input type="checkbox" class="firstQuestion" value="'+data.list[k].caseId+'">'+data.list[k].caseName+'</label></div>');
			}
			if(type == "2"){
				$('#thirdPage .answers').append('<div class="checkbox"><label><input type="checkbox" class="secondQuestion" value="'+data.list[k].caseId+'">'+data.list[k].caseName+'</label></div>');
			}
			if(type == "3"){
				$('#secondPage .answers').append('<div class="checkbox"><label><input type="checkbox" class="thirdQuestion" value="'+data.list[k].caseId+'">'+data.list[k].caseName+'</label></div>');
			}
		}
	}
